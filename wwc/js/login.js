...
function NextToPwdLogin() {
    ...
    if(account == "" || account == null){
        document.getElementById("errAccountLogin").innerHTML = "Cannot be empty";
        return;
    }
    if (!reg_alpha.test(account))//如果是纯数字（可能用电话作为账号登录）
    {
        if (account.length < 11 || !reg_phone.test(account)) {//判断格式
            document.getElementById("errAccountLogin").innerHTML = "Phone format error";
            return;
        }
        
        else {//判断账号是否存在
            if (teldata.indexOf(account) < 0) {
                document.getElementById("errAccountLogin").innerHTML = "Account does not exist.";
                return;
            }
            else {
        ...
        if (!reg_email.test(account)) {//格式错误
            document.getElementById("errAccountLogin").innerHTML = "Email format error";
            return;
        }
        else {//格式正确
...
function SignIn(){
    ...
    //判断为空
    if(pwd == "" || pwd == null){
        document.getElementById("errPwdLogin").innerHTML = "Cannot be empty";
        return;
    }
    if (pwddata[index] != pwd){//密码错误
        document.getElementById("errPwdLogin").innerHTML = "Password error.";
        return; 
    }
    else {//登录成功
        alert("^_^登录成功！");
